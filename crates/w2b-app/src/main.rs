...
fn draw_note(ui: &mut egui::Ui, player: &DraftPlayer, edits: &mut Vec<Edit>) {
    ...
    ui.horizontal(|ui| {
        for (verdict, glyph, color) in [(1i8, UP, theme::GREEN), (-1, DOWN, theme::RED)] {
            ...
            let text = egui::RichText::new(glyph).size(15.0).color(match held {
                true => color,
                false => theme::DIM,
            });
...
fn draw_hero(ui: &mut egui::Ui, hero: &w2b_core::HeroRow, most: u32, min_games: u32) {
