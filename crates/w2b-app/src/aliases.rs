...
//! gets their battletag, so the screen says `Fulano de Tal` and the battlelobby says
//! `Fulano`. Those are the seats worth reading most: the people played with are the
...
mod tests {
    ...
    fn a_drawn_name_finds_the_player_it_belongs_to() {
        let mut a = aliases();
        assert!(a.record("FulanoDeTal", "Fulano#11833"));
        let found: Vec<(String, String)> = a.pairs().collect();
        assert_eq!(
            found,
            vec![("FulanoDeTal".to_string(), "Fulano#11833".to_string())]
        );
    }
    ...
    fn a_player_keeps_only_their_newest_drawn_name() {
        let mut a = aliases();
        a.record("FulanoDeTal", "Fulano#11833");
        a.record("FulanoDe", "Fulano#11833");
        let found: Vec<String> = a.pairs().map(|(drawn, _)| drawn).collect();
        assert_eq!(found, vec!["FulanoDe".to_string()]);
    }
    ...
    fn one_friend_does_not_displace_another() {
        let mut a = aliases();
        a.record("FulanoDeTal", "Fulano#11833");
        a.record("SicranaDeTal", "sicrana#2201");
        assert_eq!(a.len(), 2);
    }
}
