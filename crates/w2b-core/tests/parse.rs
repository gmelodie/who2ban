...
/// Point `W2B_TEST_REPLAY` at a `.StormReplay` to check the parser against a real file.
#[test]
fn reads_a_real_replay() {
    let Ok(path) = std::env::var("W2B_TEST_REPLAY") else {
        return;
    };
...
fn a_new_build_falls_back_to_the_newest_protocol() {
    use w2b_parse::parse::{is_exact_build, protocol_for};
