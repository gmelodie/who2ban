...
impl Db {
    ...
    pub fn local_heroes(&self, battletag: &str, all_modes: bool) -> Result<Vec<LocalHero>> {
        ...
        let conn = self.lock();
        let sql = "SELECT coalesce(mp.hero_id, mp.hero), count(*), sum(mp.won)
        ...
        let counted: Vec<(String, u32, u32)> = rows.collect::<std::result::Result<Vec<_>, _>>()?;
        drop(stmt);

        let names = spellings(&conn)?;
        Ok(counted
            .into_iter()
            .map(|(key, games, wins)| LocalHero {
                hero: names.get(&key).cloned().unwrap_or(key),
                games,
                wins,
            })
            .collect())
...
fn write_players(tx: &rusqlite::Transaction<'_>, id: i64, replay: &MatchRecord) -> Result<()> {
    for p in &replay.players {
        tx.execute(
            "INSERT OR REPLACE INTO match_players(match_id, handle, name, battletag, hero, hero_id, team, won)
