...
/// What this program was called before it was called who2ban. A machine that ran the old
/// build keeps its database and its login here.
const FORMER_DIR: &str = "hots-draft";
const FORMER_DB: &str = "hots.db";

pub fn data_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("W2B_DATA_DIR") {
        return PathBuf::from(dir);
    }
    let root = dirs::data_dir().unwrap_or_else(std::env::temp_dir);
    let here = root.join("who2ban");
    if !here.exists() {
        carry_over(&root.join(FORMER_DIR), &here);
    }
    here
}

/// Moved once, on the first launch that goes looking for it. A rename under the same
/// parent is a directory entry, so nothing is copied and nothing can half-arrive; a
/// rename that fails leaves the old folder exactly where it was.
fn carry_over(former: &Path, here: &Path) {
    if !former.is_dir() || std::fs::rename(former, here).is_err() {
        return;
    }
    let named = here.join(FORMER_DB);
    if named.is_file() && !here.join("who2ban.db").is_file() {
        let _ = std::fs::rename(&named, here.join("who2ban.db"));
    }
    // The write-ahead log and its index belong to the file they were named after.
    for suffix in ["-wal", "-shm"] {
        let from = here.join(format!("{FORMER_DB}{suffix}"));
        if from.is_file() {
            let _ = std::fs::rename(&from, here.join(format!("who2ban.db{suffix}")));
        }
    }
}

pub fn db_path() -> PathBuf {
    data_dir().join("who2ban.db")
}
...
pub fn temp_roots(cfg: &Config) -> Vec<PathBuf> {
    ...
    if let Ok(dir) = std::env::var("W2B_TEMP_DIR") {
...
pub fn replay_dirs(cfg: &Config) -> Vec<PathBuf> {
    ...
    if let Ok(dir) = std::env::var("W2B_REPLAY_DIR") {
        return vec![PathBuf::from(dir)];
    }
