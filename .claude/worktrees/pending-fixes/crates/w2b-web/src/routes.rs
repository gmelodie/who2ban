...
use crate::state::{App, Banner, Status};
use axum::Json;
...
pub async fn post_glyphs(State(app): State<Arc<App>>, Json(atlas): Json<Atlas>) -> Reply<Learned> {
    let learned = tokio::task::spawn_blocking(move || -> Result<Learned, String> {
        let gained = app.absorb(&atlas).map_err(|e| e.to_string())?;
    ...
    if learned.gained > 0 {
        tracing::info!(
            gained = learned.gained,
            letters = learned.letters,
...
pub async fn post_banners(
    State(app): State<Arc<App>>,
    Json(bodies): Json<Vec<BannerBody>>,
) -> Reply<Learned> {
    let sent = bodies.len();
    // Decoding and reading the letters is the server's heaviest work: keep it off the runtime.
    let learned = tokio::task::spawn_blocking(move || -> Result<Learned, String> {
        let banners: Vec<Banner> = bodies.into_iter().filter_map(decode).collect();
        let gained = app.digest_all(&banners).map_err(|e| e.to_string())?;
        let (letters, examples) = app.atlas_size();
        Ok(Learned {
            gained,
            letters,
...
fn decode(body: BannerBody) -> Option<Banner> {
    match image::load_from_memory_with_format(&body.png, image::ImageFormat::Png) {
        Ok(image) => {
        ...
        Err(e) => {
            tracing::warn!(name = %body.name, error = %e, "banner would not decode");
            None
        }
    }
}
