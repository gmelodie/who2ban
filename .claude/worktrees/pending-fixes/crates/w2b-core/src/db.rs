...
CREATE INDEX IF NOT EXISTS match_players_hero_id ON match_players(hero_id);
...
fn migrate(conn: &Connection, path: Option<&Path>) -> Result<()> {
    ...
    // Before the schema, not after: `CREATE TABLE IF NOT EXISTS` leaves an existing table
    // alone, so a column the schema now indexes has to be added by name first or the
    // index is asked for on a column that is not there yet.
    if version < 6 {
        for add in [
            "ALTER TABLE matches ADD COLUMN map_id TEXT",
            "ALTER TABLE match_players ADD COLUMN hero_id TEXT",
        ] {
            // Fails harmlessly on a database whose tables this build is about to create.
            let _ = conn.execute(add, []);
        }
    }
    if version < 7 {
        // Rows written before this build lose the flag: nobody read it off those replays.
        let _ = conn.execute(
            "ALTER TABLE match_players ADD COLUMN mvp INTEGER NOT NULL DEFAULT 0",
            [],
        );
    }
    conn.execute_batch(SCHEMA)?;
...
/// One row of the count, keyed on the hero id the query grouped on.
struct Counted {
    key: String,
    games: u32,
    wins: u32,
...
impl Db {
    ...
    pub fn record_replay(&self, path: &str, replay: &MatchRecord) -> Result<Option<i64>> {
        ...
        let id = match existing {
            ...
            None => {
                tx.execute(
                    "INSERT INTO matches(game_id, map_id, roster, played_at, map, mode)
                     VALUES(?1, ?2, ?3, ?4, ?5, ?6)",
                    params![
                        game_id,
                        replay.map_id.map(|id| id.to_string()),
                        roster,
                        replay.played_at,
                        replay.map,
                        replay.mode.as_str()
    ...
    ///
    /// Grouped on the hero's id rather than its name, because the name is whatever the
    /// client that saved the replay calls it: Deathwing and Asa da Morte are one hero and
    /// must not be two rows. Rows from before the id was read fall back to the spelling.
    pub fn local_heroes(&self, battletag: &str, all_modes: bool) -> Result<Vec<LocalHero>> {
        ...
        let conn = self.lock();
        let sql = "SELECT coalesce(mp.hero_id, mp.hero), count(*), sum(mp.won), sum(mp.mvp)
        ...
        let rows = stmt.query_map(params![battletag, all_modes, name], |r| {
            Ok(Counted {
                ...
                mvps: r.get::<_, i64>(3)? as u32,
            })
        })?;
        let counted: Vec<Counted> = rows.collect::<std::result::Result<Vec<_>, _>>()?;
        drop(stmt);

        let names = spellings(&conn)?;
        Ok(counted
            .into_iter()
            .map(|c| LocalHero {
                hero: names.get(&c.key).cloned().unwrap_or(c.key),
                games: c.games,
                wins: c.wins,
                mvps: c.mvps,
            })
...
/// The name to show for each hero id: the spelling the most replays used. Nobody here
/// knows English from Portuguese, so the group's own files decide what a hero is called.
fn spellings(conn: &Connection) -> Result<std::collections::HashMap<String, String>> {
    let mut stmt = conn.prepare(
        "SELECT hero_id, hero, count(*) AS seen FROM match_players
         WHERE hero_id IS NOT NULL
         GROUP BY hero_id, hero ORDER BY hero_id, seen DESC, hero",
    )?;
    let rows = stmt.query_map([], |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?)))?;
    let mut best = std::collections::HashMap::new();
    for row in rows {
        let (id, hero) = row?;
        best.entry(id).or_insert(hero);
    }
    Ok(best)
}
...
fn write_players(tx: &rusqlite::Transaction<'_>, id: i64, replay: &MatchRecord) -> Result<()> {
    for p in &replay.players {
        tx.execute(
            "INSERT OR REPLACE INTO match_players(match_id, handle, name, battletag, hero, hero_id, team, won, mvp)
