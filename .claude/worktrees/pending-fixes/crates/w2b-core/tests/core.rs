...
fn one_seed_is_one_game_however_far_the_clocks_drift() {
    ...
    assert!(
        db.record_replay("mine.StormReplay", &mine)
            .unwrap()
            .is_some()
    );
    assert!(
...
fn a_finished_replay_replaces_a_severed_one() {
    ...
    assert!(
        db.record_replay("whole.StormReplay", &whole)
            .unwrap()
            .is_none()
    );
    assert_eq!(db.match_count().unwrap(), 1);
...
fn the_same_replay_keys_the_same_from_either_folder() {
    let a = w2b_core::ingest::replay_key(std::path::Path::new("/home/me/Replays/g.StormReplay"));
    let b = w2b_core::ingest::replay_key(std::path::Path::new("/mnt/backup/g.StormReplay"));
...
fn recognises_the_game_files() {
    use std::path::Path;
    use w2b_core::paths::{is_battlelobby, is_replay};
...
fn finds_the_folders_of_a_lutris_prefix() {
    use w2b_core::paths::{wine_replay_dirs, wine_temp_roots};
...
fn finds_the_folders_of_a_bottle() {
    use w2b_core::paths::wine_temp_roots;

    let home = tempfile::tempdir().unwrap();
    let bottle = home.path().join(".local/share/bottles/bottles/w2b");
...
fn finds_documents_on_either_side_of_a_onedrive_move() {
    use w2b_core::paths::{home_replay_dirs, home_temp_roots};
...
fn finds_nothing_without_a_prefix() {
    let home = tempfile::tempdir().unwrap();
    assert!(w2b_core::paths::wine_temp_roots(Some(home.path())).is_empty());
    assert!(w2b_core::paths::wine_replay_dirs(Some(home.path())).is_empty());
    assert!(w2b_core::paths::home_replay_dirs(Some(home.path())).is_empty());
    assert!(w2b_core::paths::home_temp_roots(Some(home.path())).is_empty());
    assert!(w2b_core::paths::wine_temp_roots(None).is_empty());
}

/// A live server holds rows that must survive the column arriving under them.
#[test]
fn a_database_from_before_the_flag_keeps_its_rows() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("v6.db");
    {
        ...
        old.execute_batch(
            "CREATE TABLE matches(
                 id INTEGER PRIMARY KEY, game_id TEXT UNIQUE, map_id TEXT,
                 roster TEXT NOT NULL, played_at INTEGER NOT NULL,
                 map TEXT NOT NULL, mode TEXT NOT NULL);
             CREATE TABLE replay_files(path TEXT PRIMARY KEY, match_id INTEGER NOT NULL);
             CREATE TABLE match_players(
                 match_id INTEGER NOT NULL, handle TEXT NOT NULL, name TEXT NOT NULL,
                 battletag TEXT, hero TEXT NOT NULL, hero_id TEXT,
                 team INTEGER NOT NULL, won INTEGER NOT NULL,
                 PRIMARY KEY(match_id, handle));
             INSERT INTO matches(id, game_id, roster, played_at, map, mode)
                 VALUES(1, '7', 'roster', 1700000000, 'Cursed Hollow', 'StormLeague');
             INSERT INTO match_players(match_id, handle, name, battletag, hero, hero_id, team, won)
                 VALUES(1, '1-Hero-1-0', 'Old', 'Old#1', 'Raynor', 'Rayn', 0, 1);
    ...
    let heroes = db.local_heroes("Old#1", true).unwrap();
    assert_eq!(
        (heroes[0].games, heroes[0].wins, heroes[0].mvps),
        (1, 1, 0),
        "the old row stays, and says nothing about an MVP nobody read"
    );
}

#[test]
fn the_mvp_of_a_match_outlives_the_replay_that_carried_it() {
    let db = Db::open_memory().unwrap();
    let mut game = replay(
        GameMode::StormLeague,
        &[("Me#1", "Raynor", 0, true), ("Foe#2", "Jaina", 1, false)],
    );
    game.players[0].mvp = true;

    db.record_replay("mvp.StormReplay", &game).unwrap();

    let mine = db.local_heroes("Me#1", true).unwrap();
    assert_eq!((mine[0].games, mine[0].mvps), (1, 1));
    assert_eq!(db.local_heroes("Foe#2", true).unwrap()[0].mvps, 0);
}
...
fn ranks_the_most_played_hero_first() {
    ...
    assert_eq!((rows[0].games, rows[0].wins, rows[0].mvps), (9, 4, 1));
...
fn finds_the_temp_folder_of_a_closed_game() {
    use w2b_core::paths::{temp_beside, wine_temp_roots};
...
fn ignores_a_replay_folder_outside_a_prefix() {
    use w2b_core::paths::temp_beside;
    assert!(
...
fn an_exact_battletag_outranks_an_earlier_namesake() {
    ...
    assert_eq!(
        draft.my_team,
        Some(1),
        "slot 7 is mine, slot 0 is a stranger"
    );
    assert!(
        draft
            .players
...
/// Two clients of one group, two languages, one hero. The stats must not split.
#[test]
fn one_hero_under_two_spellings_is_one_hero() {
    let db = Db::open_memory().unwrap();
    let english = MatchRecord {
        game_id: Some(11),
        ..replay(GameMode::StormLeague, &[("Foe#1", "Deathwing", 0, true)])
    };
    let mut portuguese = MatchRecord {
        game_id: Some(22),
        ..replay(GameMode::StormLeague, &[("Foe#1", "Deathwing", 0, false)])
    };
    portuguese.played_at += 7200;
    portuguese.players[0].hero = "Asa da Morte".to_string();
    // The id is what both clients agree on; only the spelling differs.
    portuguese.players[0].hero_id = Some("Dhan".to_string());
    let mut english = english;
    english.players[0].hero_id = Some("Dhan".to_string());

    db.record_replay("en.StormReplay", &english).unwrap();
    db.record_replay("pt.StormReplay", &portuguese).unwrap();
    assert_eq!(db.match_count().unwrap(), 2, "two different games");

    let heroes = db.local_heroes("Foe#1", true).unwrap();
    assert_eq!(heroes.len(), 1, "but one hero, not one per language");
    assert_eq!((heroes[0].games, heroes[0].wins), (2, 1));
    assert!(
        heroes[0].hero == "Deathwing" || heroes[0].hero == "Asa da Morte",
        "shown under a spelling its own files actually used, got {}",
        heroes[0].hero
    );
}

/// A record from a client too old to send an id still counts, under its spelling.
#[test]
fn a_hero_without_an_id_falls_back_to_its_name() {
    let db = Db::open_memory().unwrap();
    let mut old = replay(GameMode::StormLeague, &[("Foe#1", "Muradin", 0, true)]);
    old.players[0].hero_id = None;
    old.game_id = Some(1);

    db.record_replay("old.StormReplay", &old).unwrap();
    let heroes = db.local_heroes("Foe#1", true).unwrap();
    assert_eq!(heroes.len(), 1);
    assert_eq!(heroes[0].hero, "Muradin");
    assert_eq!(heroes[0].games, 1);
}
...
fn an_older_database_leaves_a_copy_behind() {
    ...
    Db::open(&path).unwrap();
    // Named for whichever shape this build reshapes into, so the test does not have to
    // be edited every time the schema moves.
    let backup = std::fs::read_dir(dir.path())
        .unwrap()
        .filter_map(|e| e.ok())
        .find(|e| e.file_name().to_string_lossy().contains("before-v"))
